...
export default ({ veiculos }) => {
  ...
  return (
    <div className="grid gap-2 mt-4">
      ...
      {veiculos
        ?.filter((v) => (showNotPosted ? !v.posted_at : v))
        ?.map((d, i) => {
          const { titulo, descricao, img, posted_at } = d;
          return (
            <Card
              key={i}
              className="p-3 flex flex-col sm:flex-row justify-between sm:items-center"
            >
              <div className="flex gap-3">
                ...
                <div className="flex flex-col justify-center">
                  ...
                  {posted_at && (
                    <h3 className="text-xs bg-green-500 text-white font-semibold px-2 py-1 rounded-sm w-fit">
                      {moment(d.posted_at).format("DD/MM/YYYY HH:hh")}
                    </h3>
                  )}
                </div>
              </div>
